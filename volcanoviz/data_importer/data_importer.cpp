    ...
    if(infile.is_open())
    {
        ...
        for(int c = 0; c < numcat; c++)
        {
            ...
            for (int plnt_idx = 0; plnt_idx < nplants; plnt_idx++)
            {
                ...
                infile >> height;
                infile >> radius;
                MinimalPlant plnt = {x, y, height, radius, false, (bool) cat};
...
static int sql_callback_common_data_all_species(void *write_info, int argc, char ** argv, char **colnames)
{
    ...
    int tree_idx;
    float a, b;
    float alpha, maxage, maxdeadage, maxheight, maxdiam;
    char grow_period;
    ...
    float creg, mor, wetbiomass, wooddensity, logseeding, hghtdense, hghtisolate, raddense, radisolate, qval;
    std::string name;
    data_importer::treeshape draw_shape;
    data_importer::viability sun, temp, moisture, slope;
    float trunkrad;
    bool iscanopy, istree, isconifer, isherb;
    for (int i = 0; i < argc; i++)
    {
        std::string valstr;
        std::string colstr;
        ...
        */
        ...
        else if (colstr == "maxdeadage")
        {
            maxdeadage = std::stof(valstr);
        }
        else if (colstr == "maxheight")
        {
            maxheight = std::stof(valstr);
        }
        else if (colstr == "maxdiam")
        {
            maxdiam = std::stof(valstr);
        }
        ...
        else if (colstr == "slopeval")
        {
            ...
            slope.cmin = -1.0f * slope.cmax; // ensure symmetry
        }
        ...
        {
            istree = (valstr == "Tree" || valstr == "Shrub/Tree");
            isherb = (valstr == "Herb");
        }
        else if (colstr == "creg")
        {
            creg = std::stof(valstr);
        }
        else if (colstr == "mor")
        {
            mor = std::stof(valstr);
        }
        else if (colstr == "wetbiomass")
        {
            wetbiomass = std::stof(valstr);
        }
        else if (colstr == "wooddensity")
        {
            wooddensity = std::stof(valstr);
        }
        else if (colstr == "canopy")
        {
            if (valstr == "Y")
        ...
        else if (colstr == "conifer")
        {
            isconifer = (valstr == "Y");
        }
        else if (colstr == "logseeding")
        {
            logseeding = std::stof(valstr);
        }
        else if (colstr == "hghtdense")
        {
            hghtdense = std::stof(valstr);
        }
        else if (colstr == "hghtisolate")
        {
            hghtisolate = std::stof(valstr);
        }
        else if (colstr == "raddense")
        {
            raddense = std::stof(valstr);
        }
        else if (colstr == "radisolate")
        {
            radisolate = std::stof(valstr);
        }
        else if (colstr == "qval")
        {
            qval = std::stof(valstr);
        }
    }
    ...
    sp.isTree = istree;
    sp.isHerb = isherb;
    sp.isConifer = isconifer;
    sp.logseeding = logseeding;

    sp.creg = creg;
    sp.mor = mor;
    sp.wetbiomass = wetbiomass;
    sp.wooddensity = wooddensity;
    sp.shapetype = draw_shape;
    sp.hghtdense = hghtdense;
    sp.hghtisolate = hghtisolate;
    sp.raddense = raddense;
    sp.radisolate = radisolate;
    sp.qval = qval;
...
std::vector<basic_tree> data_importer::minimal_to_basic(const std::map<int, std::vector<MinimalPlant> > &plants)
{
    ...
    for (auto &speccls : plants)
    {
        ...
        for (const auto &ctree : specplants)
        {
            ...
            y = ctree.y;
            radius = ctree.r;
            basic_tree newtree(x, y, radius, ctree.h, 1.0f);
