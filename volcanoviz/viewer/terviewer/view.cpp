...
void View::projectingPoint(int sx, int sy, vpPoint & pnt)
{
    ...
    // cerr << "sx, sy = " << sx << ", " << sy << " startx, starty = " << startx << ", " << starty << " width, height" << width << ", " << height << endl;

    // unproject screen point to derive world coordinates
    realy = height + 2.0f * starty - sy; realx = sx;
    win = glm::vec3((float) realx, (float) realy, 0.0f);
    viewport = glm::vec4(startx, starty, width, height);
...
void View::projectingPointPort(int sx, int sy, vpPoint & pnt, int port[4])
{
    // opengl3.2 with glm
    glm::vec3 wrld, win;
    glm::vec4 viewport;
    ...
    // cerr << "sx, sy = " << sx << ", " << sy << " startx, starty = " << startx << ", " << starty << " width, height" << width << ", " << height << endl;
    // cerr << "PORT: startx, starty = " << vsx << ", " << vsy << " width, height" << vwdth << ", " << vhght << endl;
    ...
    // realy = vhght + 2.0f * vsy - sy; realx = sx;
    realy = vhght - sy; realx = sx;
    win = glm::vec3((float) realx, (float) realy, 0.6f);
    viewport = glm::vec4(vsx, vsy, vwdth, vhght);
...
glm::mat4x4 View::getProjMtx()
{
    ...
    if(viewtype == ViewState::PERSPECTIVE)
    {
        ...
        projMx = glm::frustum(minx, maxx, -8.0f, 8.0f, 35.0f, 100000.0f);
    }
...
bool View::load(const char * filename)
{
    ...
    if(infile.is_open() && !infile.eof())
    {
        ...
        infile >> zoomdist;
        infile >> curquat[0] >> curquat[1] >> curquat[2] >> curquat[3];
        infile.close();
        return true;
    }
    else
