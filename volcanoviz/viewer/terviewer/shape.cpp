...
void Shape::setFlatColour(GLfloat * col)
{
    int i;
...
void Shape::genSphereVert(float radius, float lat, float lon, glm::mat4x4 trm)
{
    ...
    verts.push_back(p.x); verts.push_back(p.y); verts.push_back(p.z); // position
    verts.push_back(-1.0f); verts.push_back(-1.0f); // texture coordinates
    verts.push_back(v.x); verts.push_back(v.y); verts.push_back(v.z); // normal
}
...
void Shape::genSpeedCylinderCurve(std::vector<vpPoint> &curve, float radius, float tol, int dashcnt, float gaplen, float jumptol, int slices)
{
    int i, j, lim, base, vcnt = 0;
    vpPoint s, e;
    Vector v, vfin, vstart, vrot;
    glm::mat4x4 idt, tfm;
    glm::vec3 trs, n, rot;
        ...
        for(i = 0; i < lim; i++)
        {
            ...
            else
            {
                ...
                if(v.length() > tol || i == lim-1) // ignore points that are too close together, except for last segment
                {
                    ...
                    // toggle whether or not to draw curve
                    if(v.length() > jumptol) // wrap case
                    {
                        dashon = false;
                        dashaccum = 0;
                        vcnt = 0;
...
void Shape::genSpeedCylinderCurveWedge(std::vector<vpPoint> &curve, float startradius, float endradius, float tol, int dashcnt, float gaplen, float jumptol, int slices)
{
    int i, j, lim, base, vcnt = 0;
    vpPoint s, e;
    Vector v, vfin, vstart, vrot;
    glm::mat4x4 idt, tfm;
    glm::vec3 trs, n, rot;
    float rad, angle, a, x, y, stepa = PI2 / (float) slices, dashaccum = 0.0f;
    glm::vec4 p;
    bool dashon = true;
        ...
        for(i = 0; i < lim; i++)
        {
            ...
            else
            {
                ...
                if(v.length() > tol || i == lim-1) // ignore points that are too close together, except for last segment
                {
                    ...
                    // interpolate between start and end radii
                    float t = (float) vcnt / (float) dashcnt;
                    rad = (1.0f - t) * startradius + t * endradius;
                    for (j = 0; j < slices; j++)
                    {
                        x = cosf(a) * rad;
                        y = sinf(a) * rad;
...
void Shape::genPlane(glm::mat4x4 trm)
{
    int i, base;
    Vector v;
    glm::vec4 p;
    ...
    // base verts
    vpPoint b[4], uv[4];
    b[0] = vpPoint(-0.5f, -0.5f, 0.0f);
    b[1] = vpPoint(0.5f, -0.5f, 0.0f);
    b[2] = vpPoint(0.5f, 0.5f, 0.0f);
    b[3] = vpPoint(-0.5f, 0.5f, 0.0f);
    
    uv[0] = vpPoint(0.0f, 0.0f, 0.0f);
    uv[1] = vpPoint(1.0f, 0.0f, 0.0f);
    uv[2] = vpPoint(1.0f, 1.0f, 0.0f);
    uv[3] = vpPoint(0.0f, 1.0f, 0.0f);
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        n = glm::mat3(trm) * glm::normalize(glm::vec3(v.i, v.j, v.k));
        verts.push_back(p.x); verts.push_back(p.y); verts.push_back(p.z);
        verts.push_back(uv[i].x); verts.push_back(uv[i].y); // texture coordinates
        verts.push_back(n.x); verts.push_back(n.y); verts.push_back(n.z); // normal
    }
    // counterclockwise winding
    indices.push_back(base+1); indices.push_back(base+0); indices.push_back(base+2);
...
void Shape::genMesh(std::vector<vpPoint> * points, std::vector<Vector> * norms, std::vector<int> * faces, glm::mat4x4 trm)
{
    int i, base;
    glm::vec4 p;
    glm::vec3 v;
