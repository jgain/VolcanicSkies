...
void TRenderer::updateTypeMapTexture(TypeMap* tmap, typeMapInfo tinfo, bool force)
{
  ...
  if(wd != width || ht != height)
      cerr << "TRenderer::updateTypeMapTexture: Terrain Texture mismatch" << endl;
...
void TRenderer::getViewPortDim(int port[4])
{
    GLint viewport[4];
    glGetIntegerv(GL_VIEWPORT, viewport);
    // for(int i = 0; i < 4; i++)
    //    port[i] = (int) (viewport[i]);
    for(int i = 0; i < 4; i++)
        port[i] = (int) (viewport[i] / 2);
    // std::cout << "getViewPortDim - [" << port[0] << "," << port[1] << "," << port[2] << "," << port[3] << "]\n";
}
...
void TRenderer::drawManipulators(GLuint programID, bool drawToFB)
{
    ...
    {
        float alpha = 1.0f;
        if (drawToFB) // used when drawing frame for manipulator transparency blending
        {
