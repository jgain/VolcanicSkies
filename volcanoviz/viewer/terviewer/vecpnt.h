...
public:
    ...
    /* copy constructor */
    inline vpPoint(const vpPoint &p)
    {
        x = p.x; y = p.y; z = p.z;
    }
    ...
    /* distXZ: determine the distance after dropping the y coordinate */
    inline double distXZ(vpPoint p)
    { float dx, dz;
...
public:
    ...
    /* add: component-wise addition of a point p to the current vector */
    inline void add(vpPoint& p)
    { i += p.x;
      j += p.y;
      k += p.z;
    }
    ...
    /* sub: component-wise subtraction of a point p from the current vector */
    inline void sub(vpPoint& p)
    { i -= p.x;
      j -= p.y;
      k -= p.z;
    }

    /* sub: component-wise subtraction of a vector v from the current vector */
    ...
    /* rotate: turn the vector counterclockwise in the x-y plane by an angle <a> in radians */
...
public:
    ...
    inline void reset()
    {
        ...
        min = vpPoint(tmp, 0.0f, tmp);
        max = vpPoint(-1.0*tmp, 0.0f, -1.0f*tmp);
    }
...
public:
    ...
    vpPoint min; ///< minimal vertex (bottom, left, front cornder of bounding box)
    vpPoint max; ///< maximal vertex (top, right, back corner of bounding box)
...
// toRightOfLine: given a line passing through two points (p, q) determine if a point (t) lies to the right and if so return true.
bool toRightOfLine(vpPoint p, vpPoint q, vpPoint t);

// insideRect: test whether a point (t) lies inside a rectangle defined by four point (p[4]), if so return true
bool insideRect(vpPoint * p, vpPoint t);

// formRect: create a rectangle (placing corners in p) that is offset on either side of the point <b> in the direction <dirn> by a width of <w>
//          basically a thick line
void formRect(vpPoint b, float w, Vector dirn, vpPoint * p);
