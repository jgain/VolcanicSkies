...
void rayPointDist(vpPoint start, Vector dirn, vpPoint query, float &tval, float &dist)
{
    ...
    {
        cerr << "Error rayPointDist: malformed line segment" << endl;
        dist = -1.0f;
    }
    else
    {
...
bool toRightOfLine(vpPoint p, vpPoint q, vpPoint t)
{
    ...
    impval = (q.y - p.y) * t.x - (q.x - p.x) * t.y + q.x * p.y - q.y * p.x;
    return impval > 0.0f;
}
...
bool insideRect(vpPoint * p, vpPoint t)
{
    ...
    if(toRightOfLine(p[0], p[1], t) && toRightOfLine(p[1], p[2], t) && toRightOfLine(p[2], p[3], t) && toRightOfLine(p[3], p[0], t) )
        inside = true;
    return inside;
}
...
void formRect(vpPoint b, float w, Vector dirn, vpPoint * p)
{
    ...
    left = Vector(-1.0f * dirn.j, dirn.i, 0.0f);
    left.normalize(); left.mult(w);
    right = Vector(dirn.j, -1.0f * dirn.i, 0.0f);
    right.normalize(); right.mult(w);

    left.pntplusvec(b, &p[0]);
    dirn.pntplusvec(p[0], &p[1]);
    dirn.pntplusvec(b, &p[2]);
    right.pntplusvec(p[2], &p[2]);
    right.pntplusvec(b, &p[3]);
}
