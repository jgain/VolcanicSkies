...
void testConstructors(){
    auto a = atmosphericHeatingEngine<dsType, dsSType, valType, valType2>(60.f, true);
	...
	auto p = std::make_shared<PWM::Model::planet>(PWM::Model::planet("../resources/testResources/PlanetEarth.json"));
	...
	auto b = atmosphericHeatingEngine<dsType, dsSType, valType, valType2>(60.f, true);
	b.setTerrain(t);
	b.addAirLayer(l);
...
void testMiscellany(){
	auto planetos = std::make_shared<PWM::Model::planet>(PWM::Model::planet("../resources/testResources/PlanetEarth.json"));
	
	std::shared_ptr<PWM::Model::terrain<dsType, dsSType, valType, valType2>> t = nullptr;
	auto t1 = PWM::Model::terrain<dsType, dsSType, valType, valType2>(planetos, 1022);
	auto v = std::vector<std::pair<valType, bool>>();
	for (int i = 0; i < (1022 * 1022 * 6); ++i)
	...
	std::cout << "Time taken for 10 steps of atmospheric heating for 6 faces of " << 1024 << "x" << 1024 << " grid: " << a.getRunTimePassed() << " seconds." << std::endl;
}

int main(int argc, char** argv){
	std::cout << "\nTesting class \033[1;34m'atmosphericHeatingEngine'\033[0m:" << std::endl;
	...
	std::cout << "\nTesting of class \033[1;34m'atmosphericHeatingEngine'\033[0m complete." << std::endl;
}