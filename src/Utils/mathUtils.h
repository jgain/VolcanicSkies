...
namespace PWM{
    namespace Utils{
        ...
        inline const T calcCartesianDistance(const std::tuple<T1, T2, T3>& x, const std::tuple<T4, T5, T6>& y){
            return std::sqrt(std::pow((std::get<0>(y) - std::get<0>(x)), 2) + std::pow((std::get<1>(y) - std::get<1>(x)), 2) + std::pow((std::get<2>(y) - std::get<2>(x)), 2));
        }
        ...
        /**
         * @brief getWorldLoc
         * Convert a cell index to a world location.
         * @param i The index in the x-axis
         * @param j The index in the y-axis
         * @param gridLength The cell size (assumes square cells)
         * @return A pair (x, y) that is the world location of cell (i, j).
         */
        template<typename T>
        inline const std::pair<T, T> getWorldLoc(const int i, const int j, const T gridLength){
            T x = i * gridLength;
            T y = j * gridLength;
         ...
         * @param j The index in the y-axis
         * @param gridLength The cell size (assumes square cells)
         * @param offset The offset inherent in the grid along each axis
         * @return A pair (x, y) that is the world location of cell (i, j).
         */
        template<typename T>
        inline const std::pair<T, T> getWorldLoc(const size_t i, const size_t j, const T gridLength, const std::pair<T, T>& offset){
            T x = i * gridLength + offset.first;
            T y = j * gridLength + offset.second;
            return std::make_pair(x, y);
        }

        /**
         * @brief getCoordLoc
         * Convert a world location to the nearest cell index.
         * @param x The world location along the x-axis.
         * @param y The world location along the y-axis.
         * @param gridLength The cell size (assumes square cells)
         * @return A pair (i, j) that is the cell closest to the world location (x, y)
         */
        template<typename T>
        inline const std::pair<size_t, size_t> getCoordLoc(const T x, const T y, const T gridLength){
            size_t i = std::round(x / gridLength);
            size_t j = std::round(y / gridLength);
         ...
         */
        template<typename T>
        inline const std::pair<size_t, size_t> getCoordLoc(const T x, const T y, const T gridLength, const std::pair<T, T>& offset){
            size_t i = std::round(x / gridLength - offset.first);
            size_t j = std::round(y / gridLength - offset.second);
            return std::make_pair(i, j);
        }
