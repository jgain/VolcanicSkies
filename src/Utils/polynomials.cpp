...
#include "polynomials.h"		   // solution of cubic and quartic equation
	...
	namespace Utils{
		...
		int SolveP3(float *x,float a,float b,float c) {	// solve cubic equation x^3 + a*x^2 + b*x + c = 0
			...
			if (r2 <= (q3 + eps)) {//<<-- FIXED!
				...
				a/=3;
				q=-2*sqrt(q);
				x[0] = (float) (q*cos(t/3)-a);
				x[1] = (float) (q*cos((t+TwoPi)/3)-a);
				x[2] = (float) (q*cos((t-TwoPi)/3)-a);
				return(3);
			} else {
				//A =-pow(fabs(r)+sqrt(r2-q3),1./3); 
				A = -root3(fabs(r)+sqrt(r2-q3)); 
